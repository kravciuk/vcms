...
urlpatterns = [
    ...
    url(r'^(?P<short_id>[A-Za-z0-9]+)/(?P<content_type>(raw))/$', view_snippet, name='source'),
    url(r'^(?P<short_id>[A-Za-z0-9]+)/download/(?P<enc_key>[A-Za-z0-9_-]+)/$', download_file, name='download'),
