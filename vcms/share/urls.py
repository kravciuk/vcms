...
urlpatterns = [
    ...
    url(r'^(?P<short_id>[A-Za-z0-9]+)/forbidden/$', download_forbidden, name='share_download_forbidden'),
    url(r'^(?P<short_id>[A-Za-z0-9]+)/(?P<content_type>(raw))/$', view_snippet, name='share_raw_snippet'),
    url(r'^(?P<short_id>[A-Za-z0-9]+)/download/(?P<enc_key>[A-Za-z0-9_-]+)/$', download_file, name='share_download_file'),
